...
import { FileText, Brain, Users, AlertTriangle, TrendingUp, Filter, MessageSquare, Save, X } from "lucide-react";
  ...
  const saveReportUpdate = async () => {
    ...
    const { error } = await supabase
      ...
      .update({
        ...
        counselor_recommendation: counselorRec,
        status: newStatus,
      })
      .eq("id", selectedReport.id);

    if (error) {
      toast.error("خطأ في حفظ التحديث");
    } else {
      toast.success("تم حفظ التحديث بنجاح");
          ...
          <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setSelectedReport(null)}>
            ...
            >
              <div className="flex items-center justify-between mb-4">
                ...
                <button onClick={() => setSelectedReport(null)} className="p-1 rounded-lg hover:bg-muted">
                  <X className="w-5 h-5" />
                </button>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3 mb-4 text-small">
                ...
                <div className="bg-muted/50 rounded-lg p-3">
                  ...
                  <p className="font-bold">{selectedReport.emotion || "—"}</p>
                </div>
                <div className="bg-muted/50 rounded-lg p-3">
                  <span className="text-muted-foreground">الدور:</span>
                  <p className="font-bold">{selectedReport.role === "student" ? "طالب" : "معلم"}</p>
                </div>
                <div className="bg-muted/50 rounded-lg p-3">
                  <span className="text-muted-foreground">المرحلة:</span>
                  <p className="font-bold">{selectedReport.education_level || "—"}</p>
                </div>
              </div>
              ...
              {selectedReport.ai_recommendations && selectedReport.ai_recommendations.length > 0 && (
                <div className="bg-accent/5 border border-accent/20 rounded-lg p-4 mb-4">
                  <h3 className="text-btn font-bold text-accent mb-2 flex items-center gap-2">
              ...
              <div className="space-y-4 border-t border-border pt-4">
                <h3 className="text-btn font-bold flex items-center gap-2">
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full p-2.5 rounded-lg border border-border bg-card text-body"
                  >
                ...
                <div>
                  ...
                  <textarea
                    value={counselorRec}
                    onChange={(e) => setCounselorRec(e.target.value)}
                    placeholder="أضف توصيتك الشخصية..."
                    rows={3}
                    className="w-full p-3 rounded-lg border border-border bg-card text-body resize-none"
                  />
                </div>
                ...
                <button
                  onClick={saveReportUpdate}
                  disabled={saving}
                  className="w-full flex items-center justify-center gap-2 px-6 py-3 rounded-lg bg-primary text-primary-foreground text-btn font-bold disabled:opacity-50"
                >
